...
class CiphertextReencoderTest : public ::testing::Test {
   protected:
    void SetUp() {
        ...
        encryptor_ =
            make_unique<Encryptor>(seal_context_, keygen_->public_key());
        decryptor_ =
            make_unique<Decryptor>(seal_context_, keygen_->secret_key());
...
TEST_F(CiphertextReencoderTest, TestRecursion) {
    ...
    for (size_t i = 0; i < exp_ratio; ++i) {
        auto result_ct = ct_reencoder_->Decode(
            pt_lvl_2.begin() + (i * exp_ratio), ct.size());
