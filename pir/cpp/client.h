...
class PIRClient {
   ...
   public:
    ...
    /**
     * Creates a new request to query the database for the given index. Note
     *that if more than one dimension is specified in context, then the request
     * generated will include multiple selection vectors concatenated into one
     *set of ciphertexts. It is expected that the server will first expand the
     ...
     * @returns InvalidArgument if the index is invalid or if the encryption
     *fails
     **/
   ...
   private:
    ...
    Status initialize();
    Status createQueryFor(size_t desired_index,
    ...
    StatusOr<seal::Plaintext> ProcessReply(
