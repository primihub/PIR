...
class PIRServerTestBase : public PIRTestingBase {
   protected:
    void SetUpDBImpl(size_t dbsize, size_t dimensions = 1,
        ...
        gal_keys_ = keygen_->galois_keys_local(
...
class PIRServerTest : public ::testing::TestWithParam<bool>,
   ...
   protected:
    ...
    void SetUpDB(size_t dbsize, size_t dimensions = 1,
                 size_t elem_size = ELEM_SIZE,
                 uint32_t plain_mod_bit_size = 20) {
        SetUpDBImpl(dbsize, dimensions, elem_size, plain_mod_bit_size,
                    GetParam());
    }
};
...
TEST_P(PIRServerTest, TestProcessRequest_2Dim) {
    ...
    ASSIGN_OR_FAIL(
        auto reply,
        LoadCiphertexts(server_->Context()->SEALContext(), response.reply(0)));
    ...
    if (GetParam()) {
        ...
        ASSIGN_OR_FAIL(
            auto ct_reencoder,
            CiphertextReencoder::Create(server_->Context()->SEALContext()));
        ASSERT_THAT(reply,
