...
EncryptionParameters GenerateEncryptionParams(
    ...
    auto poly_modulus_degree =
        poly_mod_opt.value_or(DEFAULT_POLY_MODULUS_DEGREE);
...
StatusOr<shared_ptr<PIRParameters>> CreatePIRParameters(
    ...
    parameters->set_use_ciphertext_multiplication(
        use_ciphertext_multiplication);

    if (bits_per_coeff > 0) {
        if (bits_per_coeff > encoder.bits_per_coeff()) {
            return InvalidArgumentError(
                "Bits per coefficient greater than max");
        }
    ...
    if (bytes_per_item > 0) {
        ...
        if (parameters->items_per_plaintext() <= 0) {
            return InvalidArgumentError(
                "Cannot fit an item within one plaintext");
        }
