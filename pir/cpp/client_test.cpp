...
class PIRClientTest : public ::testing::Test {
   protected:
    ...
    void SetUpDB(size_t dbsize, size_t dimensions = 1, size_t elem_size = 0,
        ...
        pir_params_ = *(CreatePIRParameters(dbsize, elem_size, dimensions,
                                            encryption_params_,
...
TEST_F(PIRClientTest, TestCreateRequest) {
    ...
    ASSIGN_OR_FAIL(auto req, LoadCiphertexts(Context()->SEALContext(),
                                             req_proto.query(0)));
...
TEST_F(PIRClientTest, TestCreateRequestD2) {
    ...
    EXPECT_EQ((pt[expected_row] * next_power_two(total_s_items)) % plain_mod,
              1);
    EXPECT_EQ((pt[num_rows + expected_col] * next_power_two(total_s_items)) %
                  plain_mod,
              1);
    for (size_t i = 0; i < pt.coeff_count(); ++i) {
...
TEST_F(PIRClientTest, TestCreateRequestMultiDimMultiCT2) {
    ...
    const size_t m =
        next_power_two((num_rows + num_cols) % POLY_MODULUS_DEGREE);
